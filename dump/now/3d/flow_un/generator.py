...
#---------------------------------------------------------------------
#---------------------------------------------------------------------
###### random, adjusted for evaluation ##############################

class ImageSequence_eval(Sequence):
    def __init__(self,  batch_size=2, input_size=(436, 1024),f_gap=1):
        self.image_seq_path="/media/newhd/data/flow/MPI_SINTEL/MPI-Sintel-complete/training/albedo/"
        self.flow_seq_path="/media/newhd/data/flow/MPI_SINTEL/MPI-Sintel-complete/training/flow/"
        self.mask_seq_path="/media/newhd/data/flow/MPI_SINTEL/MPI-Sintel-complete/training/invalid/"
        self.occ_seq_path="/media/newhd/data/flow/MPI_SINTEL/MPI-Sintel-complete/training/occlusions/"

        self.input_shape=input_size

        self.im_dirs=os.listdir(self.image_seq_path)
        self.fl_dirs=os.listdir(self.flow_seq_path)
        self.msk_dirs=os.listdir(self.mask_seq_path)
        self.occ_dirs=os.listdir(self.occ_seq_path)
        ...
        self.len_im_dirs=len(self.im_dirs)
        self.len_fl_dirs=len(self.fl_dirs)
        self.len_msk_dirs=len(self.msk_dirs)
        self.len_occ_dirs=len(self.occ_dirs)
...
###########--------------------------------------
    ...
    def __getitem__(self, idx = None):
        ...
        im_path = self.image_seq_path+self.im_dirs[d_idx]+"/"
        fl_path = self.flow_seq_path+self.fl_dirs[d_idx]+"/"
        msk_path = self.mask_seq_path+self.msk_dirs[d_idx]+"/"
        occ_path = self.occ_seq_path+self.occ_dirs[d_idx]+"/"
        ...
        self.msk_frames = os.listdir(msk_path)
        self.msk_frames.sort()
        num_msk_frames = len(self.msk_frames)

        self.occ_frames = os.listdir(occ_path)
        self.occ_frames.sort()
        num_occ_frames = len(self.occ_frames)
        ...
        while(c<self.batch_size):
            ...
            I2_file = self.im_frames[s_idx+self.f_gap]
            flow_file = self.fl_frames[s_idx]
            mask_file = self.msk_frames[s_idx]
            occ_file = self.occ_frames[s_idx]

            c = c+1
            I1=self.read_image(im_path+I1_file)
            I2=self.read_image(im_path+I2_file)
            flo = self.read_flow(fl_path+flow_file)
            mask = self.read_image(msk_path+mask_file)
            occ = self.read_image(occ_path+occ_file)
            ...
            occ_batch.append(occ)

        x_batch = np.array(x_batch, np.float32)
        flow_batch = np.array(flow_batch) 
        mask_batch = np.array(mask_batch)
        occ_batch = np.array(occ_batch)
        ...
        return ([x_batch[:,0,:,:,:],x_batch[:,1,:,:,:]],flow_batch,mask_batch,occ_batch)
        # return ([x_batch[:,0,:,:,:],x_batch[:,1,:,:,:]],flow_batch)
        # return ([x_batch[:,0,:,:,:], x_batch[:,1,:,:,:]],None)
